$(function () {
        ...
        if(windowWidth > 0) {
            if ($(document).scrollTop() >= 100) {
                ...
                if($('.do-sticky').length < 1) {
                    $('.navbar-brand.logo img').attr('src', 'assets/img/logos/black-logo.png');
                }
            }
            else {
                $('.sticky-header').removeClass('header-shrink');
                if($('.do-sticky').length < 1 && $('.fixed-header').length == 0 && $('.fixed-header2').length == 0) {
                    $('.navbar-brand.logo img').attr('src', 'assets/img/logos/logo.png');
                } else {
                    $('.navbar-brand.logo img').attr('src', 'assets/img/logos/logo.png');
                }
            }
   ...
   $(".dropdown").on('hover', function () {
    ...
    // Magnify activation
    $('.item-magnify-gallery').each(function() {
        $(this).magnificPopup({
            delegate: 'a',
            type: 'image',
    ...
    if($('.player').length > 0){
        $(document).on('ready', function () {
            $(".player").mb_YTPlayer({
                mobileFallbackImage: 'assets/img/banner-1.jpg'
            });
        });
    }
